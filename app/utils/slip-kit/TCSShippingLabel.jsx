...
const styles = StyleSheet.create({
  page: { width: LABEL_WIDTH, flexDirection: "column", backgroundColor: "white", padding: 10, height: "100%", fontFamily: "Roboto", fontSize: 9 },
    ...
    <View style={styles.section}>
      <View style={styles.row}>
        <View style={[styles.column, { width: "25%", padding: 8 }]}>
          {order.courier_logo_url ? <Image src={order.courier_logo_url} style={{ height: 48, alignSelf: "center" }} /> : null}
          <Text style={[styles.bold, { textAlign: "center" }]}>TCS (Pvt) Ltd</Text>
        </View>
        <View style={[styles.column, styles.center, { width: "30%", padding: 8 }]}>
          {order.barcodes?.tracking_number ? <Image src={order.barcodes.tracking_number} style={{ width: 140, height: 50, alignSelf: "center" }} /> : null}
          <Text style={[styles.bold, { textAlign: "center" }]}>Consignee Copy</Text>
        </View>
        <View style={[styles.column, { width: "25%" }]}>
          <View style={[styles.row, styles.borderBottom]}>
            <Text style={[styles.bold, { width: "100%", padding: 8 }]}>Date: {new Date(order.fulfillment_date || Date.now()).toLocaleDateString()}</Text>
          </View>
          <View style={[styles.row, styles.borderBottom]}>
            <Text style={[styles.bold, { width: "100%", padding: 8 }]}>{order.service_type || "Express"}</Text>
          </View>
          <View style={styles.row}>
        ...
        <View style={[styles.column, styles.center, { width: "20%", padding: 8 }]}>
          {order.store_logo_url ? <Image src={order.store_logo_url} style={{ width: 70, height: 70 }} /> : null}
        </View>
      </View>
      ...
      <View style={styles.row}>
        ...
        <View style={[styles.column, { width: "60%", padding: 8, borderRight: 0 }]}>
          ...
          <Text style={{ marginBottom: 6 }}>{(order.destination_address?.address1 || "") + " " + (order.destination_address?.address2 || "")}</Text>
      ...
      <View style={styles.row}>
        <View style={[styles.column, { width: "40%" }]}>
          <View style={[styles.row, styles.borderBottom]}>
            <View style={{ width: "60%", borderRight: "1pt solid black" }}>
              ...
              <View style={{ height: 40, padding: 4 }}><Text>Declared Insurance Value</Text></View>
            </View>
            <View style={{ width: "40%" }}>
              <View style={[styles.row, styles.borderBottom]}>
        ...
        <View style={[styles.row, { width: "60%" }]}>
          ...
          <View style={[styles.column, { width: "30%", padding: 8 }]}><Text style={styles.bold}>COD AMOUNT</Text></View>
          <View style={[styles.column, styles.center, { width: "40%", borderRight: 0 }]}>
            {order.barcodes?.cod_amount ? <Image src={order.barcodes.cod_amount} style={{ width: 100, height: 20 }} /> : null}
          </View>
        </View>
      </View>

      <View style={[styles.row, { width: "100%" }]}>
        <Text style={[styles.bold, { borderRight: "1pt solid black", width: "20%", padding: 2 }]}>Product Detail</Text>
        <Text style={{ width: "80%", padding: 2 }}>{order.product_details || "No Product Details"}</Text>
      </View>

      <View style={[styles.row, { width: "100%" }]}>
        <Text style={[styles.bold, { borderRight: "1pt solid black", width: "20%", padding: 2 }]}>Remarks</Text>
      ...
      <View style={styles.row}>
        <View style={[{ width: "10%" }]}>
          <View style={[styles.column, styles.center]}>
            {order.qr_codes?.details ? <Image src={order.qr_codes.details} style={{ width: 40, height: 40 }} /> : null}
          </View>
        </View>
        <View style={[{ width: "25%" }]}>
          <View style={[styles.column, styles.center, { padding: 8, width: "100%" }]}>
            {order.barcodes?.tcs_third_barcode ? <Image src={order.barcodes.tcs_third_barcode} style={{ width: 110, height: 40 }} /> : null}
          </View>
        </View>
        ...
        <View style={[{ width: "40%", padding: 4 }]}>
          <Text style={{ fontSize: 9 }}>
            Please don't accept if shipment is not intact. Before paying the COD, shipment cannot be opened. For complaints, contact {order.shipper_details?.name || ""} Ph: {order.shipper_details?.phone || ""}
          </Text>
        </View>
      </View>
    </View>
